function s = RBF_PU(X, Y, IndXY, IndYY, fX, rho, op,RBFinfo)
...
%  op: operator
%  RBFinfo: information of RBF (par, type, poly order, ...)
% Output:
...
for j = 1:Nc
  ...
  if ~isempty(inds)
    ...
    Lmat = LagrangeMat(Xhat,Xj,yj,rho(j),op,ScaleOrd,RBFinfo); % RBF Lagrange matrix
    for k=1:numop
        sj = Lmat{k}*fXj;
